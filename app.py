...
def show_results():
    ...
    with left:
        st.markdown(f"""
            ...
            ">
                <div style="margin:0 0 .5rem 0; font-size:2rem; color:#1d3557;">
    ...
    with right:
        ...
        with card:
            st.markdown(f"""
                <div style="font-size:2rem; font-weight:800; margin-bottom:.7rem; color:{color};">
                    {title}
                </div>
            """, unsafe_allow_html=True)
            ...
            with text_col:
                st.markdown(
                    f"<div style='font-weight:800; font-size:1.5rem; color:#1d3557;'>{arc_name}</div>",
                    unsafe_allow_html=True
                )
                st.markdown(
                    f"<div style='margin-top:.45rem; font-size:1.5rem; color:#1b4332;'>Top impact area: <b>{actual_top}</b></div>",
                    unsafe_allow_html=True
                )
            with img_col:
                if arc_img:
